...
## TD-4 · LLM providers: Gemini + Groq + OpenRouter
...
**Why (OpenRouter, added M8, 2026-07-22):** added as a consistency lever against Groq's `gpt-oss-120b` reasoning-model chain-of-thought leaking into replies — a meta-provider with a documented plain-chat default sidesteps that failure mode entirely. **`httpx` accepted over the `openai` SDK** (amendment): OpenRouter speaks the same OpenAI-Chat-Completions dialect Groq already does, so the adapter reuses `_openai_compat.py` — pulling in the `openai` SDK for one more adapter would duplicate what `httpx` (already justified, TD-11) does in ~150 lines. Default model **`nvidia/nemotron-3-super-120b-a12b:free`** (verified 2026-07-22), confirmed tool-calling-capable.
**Free-tier reliability follow-up (2026-07-23):** the `:free` tier's backend load-balancing produced observed degenerate output (`<unk>`-token spam) from a low-precision quantized backend. Mitigated two ways rather than abandoning the free tier: a `provider.quantizations` allow-list on every OpenRouter request (excludes `int4`/`fp4`/`fp6`/`unknown`; docs/10 §2.3), and a provider-agnostic degenerate-output detect-and-retry-once in `agent.py`. Switching the default off `:free` (a paid model) was considered and rejected for now — it would remove the failure mode outright but costs money per call for a mitigation that's unproven to be otherwise necessary; revisit if the quantization pin + retry prove insufficient in practice.
